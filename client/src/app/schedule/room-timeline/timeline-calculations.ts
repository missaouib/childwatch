...
export function slots(people: Person[], timeline: PersonTimeline, roomId: string, now: DateType): TimelineGridSlot[] {
  ...
  for (const person of _.values(timeline)) {
    const schedule = _.filter(person.schedule, { roomId });
...
export function calculateTimeline(schedule: Schedule[], presence: Presence[], now: DateType): GraphTimeSegment[] {
...
export function calculateTimeEvents(schedule: Schedule[], presence: Presence[], now: DateType): TimeEvent[] {
