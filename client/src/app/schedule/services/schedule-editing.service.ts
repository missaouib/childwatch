...
import {
  ...
  scheduleEditorDateChanged, updateSchedule, UPDATE_SCHEDULE
} from '../scheduling.state';
// import { SchedulingQueryService } from './scheduling-query.service';
  ...
  updateSchedule$ = this.actions$.ofType(UPDATE_SCHEDULE)
    ...
    .concatMap(([newSchedule, personSchedule]) => {
      ...
      if (personSchedule.type === PersonType.Participant) {
        mutation = undefined; //updateParticipantSchedule;
        variables.participantId = personSchedule.who.id;
      } else {
        mutation = undefined; //updateStaffSchedule;
      ...
      return undefined; // this.apollo.mutate({
//        mutation,
//        variables,
